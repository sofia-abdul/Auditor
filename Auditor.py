...
class App(customtkinter.CTk):
    ...
    def login(self):
        ...
        if username == "admin" and password == "password":
            ...
            self.progress_bar = customtkinter.CTkProgressBar(self.login_frame, mode="indeterminate", width=300)
            self.progress_bar.pack(pady=12, padx=10)
            self.progress_bar.start()
            self.update()
            threading.Thread(target=self.login_delay_thread, daemon=True).start()
        else:
            self.login_error_label.configure(text="Invalid username or password")

    def login_delay_thread(self):
        try:
            time.sleep(3) #Simulate 2 second login delay
            self.after(0, lambda: self.progress_bar.stop())
            self.after(0, lambda: self.progress_bar.pack_forget())
            self.after(0, self.login_frame.destroy)
            self.after(0, self.show_main_app)
        
        except Exception as e:
            self.after(0, lambda: self.login_error_label.configure(text=f"Initialization failed: {e}"))
            self.after(0, lambda: self.progress_bar.stop())
            self.after(0, lambda: self.progress_bar.pack_forget())
            self.after(0, lambda: self.login_button.pack(pady=12, padx=10))
    ...
    def show_main_app(self):
        ...
        self.geometry("1200x600")  # Resize for dashboard
        self.grid_columnconfigure(0, weight=1)  # Adjust grid for dashboard
    ...
    def create_dashboard_frame(self):
        ...
        self.dashboard_frame.grid_columnconfigure(1, weight=0)
        ...
        logout_button = customtkinter.CTkButton(
            self.dashboard_frame, text="Log-Out", height=30, command=self.logout,
            font=customtkinter.CTkFont("Times New Roman", size=15, weight="bold")
        )
        logout_button.grid(row=0, column=0, padx=20, pady=(10, 10), sticky="e")
    ...
    def update_plot(self, frame):
        ...
        if now - state['start_time'] < state['warmup_time']:
            ...
            if len(state['radar_buffer']) >= 5 and sum(state['radar_buffer'][-5:]) >= 4:
